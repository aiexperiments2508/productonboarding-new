...
The bar along the bottom & A date reading \ui{2026-07-01} and a counter
reading \ui{0/5211}. \\
\bottomrule
\end{longtable}
...
\begin{say}[Say this]
``That is two months of supplier traffic --- the first of July to the thirty-first
of August, replayed off a snapshot the way a flight recorder is read back.
Five thousand two hundred events across a hundred and fifty products,
delivered by eleven systems in batches, at irregular times, overlapping each
other --- which is what a real morning looks like.

``The systems are now reporting how much they sent, and how much of it was
defective.''
\end{say}

\textbf{Do this:} point at \ui{THE ESTATE} panel. It now shows counts. Then
...
\textbf{Do this:} click \ui{The record} in the strip under the product name.
...
\textbf{Do this:} click \ui{Northaven AP300} at the top of the list --- a green
one. Then click \ui{Open staging page}, top right.
...
\begin{careful}[The staging page records who looked]
The \ui{Viewing as} box beside the button is deliberate --- this is unpublished
commercial content, and the system records who saw it before it launched. It is
filled in for you; change it to your own name if you like.
\end{careful}

% ---------------------------------------------------------------------------
\subsection{If somebody asks ``does it only do allergens?''}

They will, and the honest answer is a list. The tape carries seven more
corrections on the rest of the catalogue, each a different \emph{kind} of
thing going wrong. Use \ui{Jump to the correction} and then step the transport
to the day, or just say what they are.

\begin{center}
\begin{tabular}{@{}rll@{}}
\textbf{Day} & \textbf{What arrives} & \textbf{Why it is different} \\
\midrule
8  & A pack shrinks & shelf label and price per unit both quote it \\
22 & A certificate expires & product unchanged, the claim loses its evidence \\
27 & Country of origin moves & a labelling particular and a claim at once \\
44 & \textbf{A withdrawal notice} & not a correction --- nothing is wrong, it stops \\
48 & An export classification & lawful here, restricted elsewhere \\
52 & A fibre label is revised & ordered declaration, so a reorder is a change \\
56 & The rules themselves change & nobody's fault, nothing to return \\
\end{tabular}
\end{center}

\begin{say}[Say this, on day 44]
``This one is worth a second. Everything else we have looked at was a value
being wrong. Here nothing is wrong --- a market authority has said stop, and
the record is perfect. The system takes every channel down on the authority of
the notice, and it does not pretend it can recall the printed catalogue. That
is an erratum, owed and dated.''
\end{say}

\begin{careful}[Two of these are not safety issues, and saying so matters]
The export classification on day 48 and the lapsed certificate on day 22 both
sound alarming and neither is a safety finding. If somebody in the room hears
``dual-use'' and reaches for the word ``unsafe'', correct it --- the product is
fine, and we simply cannot ship it everywhere.
\end{careful}
...
about the eleven systems is written into the application.''
\end{say}
...
\begin{say}
``Through an open standard called MCP. Each of the eleven systems is a separate
...
Check: green \ui{gateway} dot, \ui{The catalog is clean}, counter \ui{0/5211}
\end{tcolorbox}

\vspace{0.4em}
\begin{tcolorbox}[colback=white,colframe=accent,boxrule=1pt,arc=2pt]
\textbf{The five acts}\\[4pt]
\textbf{1.} \ui{Ingest Fabric} --- ``eleven systems, catalogue is clean'' \\[3pt]
...
score'' --- click a green one, \ui{Open staging page}
\end{tcolorbox}
