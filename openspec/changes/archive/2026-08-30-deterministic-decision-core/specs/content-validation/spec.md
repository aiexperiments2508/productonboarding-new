...
- **THEN** blocking violations are raised on all five channels, each naming the
  attribute with the threshold as required and 0.62 as available, the result is
  not publishable, and the safety-flag count is non-zero
- **AND** `tests/test_validator.py::test_low_confidence_on_a_safety_attribute_blocks_every_listing`
  asserts each
