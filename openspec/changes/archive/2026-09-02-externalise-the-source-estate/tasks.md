...
- [x] 5.2 Connect by address with a real handshake, recording what the system
      answered as one record per address; verify via
      `tests/test_connections.py::test_connecting_the_same_address_twice_updates_one_record`
      and `::test_disconnecting_then_reconnecting_leaves_one_connection`. The
      answering-server path is not covered: the suite runs with nothing
      listening, so 5.3 is the path it exercises
...
- [x] 6.4 Emit topology changes on a stream that agrees with the listing; verify
      via `tests/test_connections.py::test_the_topology_stream_and_the_listing_agree`.
      That one message is published per change is by inspection of the two route
      bodies rather than by a test
