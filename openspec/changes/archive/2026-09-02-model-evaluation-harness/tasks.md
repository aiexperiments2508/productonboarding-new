...
- [x] 1.1 Emit `data/golden/extractions.jsonl` from the generator, one row per
      material document the extractor is asked to read and a sample of the
      immaterial ones, out of the same event payloads the prose was written
      from; verify via
...
- [ ] 4.1 Update the README: it reports 866 tests where the suite is 868, and it
      has no evaluation section at all - no mention of what the harness
