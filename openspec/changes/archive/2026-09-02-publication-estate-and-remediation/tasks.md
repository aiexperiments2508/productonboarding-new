...
- [x] 5.1 Serve each publication system as its own MCP endpoint at
      `/mcp/publish/{channel}`, serving exactly the tools it declares with the
      mutating set declared rather than inferred from the verbs - a count in the
      assertion would go wrong the moment the surface grew; verify via
