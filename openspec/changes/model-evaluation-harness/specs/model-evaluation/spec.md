...
and SHALL be rewritten whenever the data is. It SHALL cover exactly the
documents the extractor is asked to read, in the order the tape delivers them,
and no others - the arrivals that are already structured are never put in front
of a model and grading them would flatter the result with documents nobody asks
a model to read.
...
- **THEN** they are the same list in the same order
- **AND** `tests/test_golden.py::test_key_covers_exactly_the_documents_extract_reads`
