...
**Three lanes.** Everything above describes ``lane = 'TAPE'``: the recorded
...
A third, ``REF``, holds reference data: what a depot holds, what a market
charged, which campaigns ran, what a certificate register says. Four
back-office systems deliver it once at bootstrap and then it is simply there.
It is not part of the recording, so the transport must not count it in its
denominator, play it, or lose it on a rewind; and it is not a submission, so
the live feed must not announce it. Both fall out of it having its own lane
rather than out of anybody remembering to exclude it - which is the whole
argument for adding a lane instead of a high sequence band and a filter.
...
That one widens to ``LIVE`` by name and is bounded by the cursor otherwise, so
it does not reach ``REF`` either.

The rule generalises: a query in this module states the lane it means. One that
does not is a query that will be wrong the next time a lane is added, and it
...
#: Where reference sequence numbers start, above the live band for the same
#: reason the live band sits above the tape: so a row's origin is obvious in a
#: log line without looking it up. Also a numbering convention and never a
#: predicate - `released` is bounded by the tape cursor and names `LIVE`
...
def load_reference(path: Path | None = None, reset: bool = False) -> dict:
    """Load the back-office reference pack. Idempotent unless ``reset``.

    Four systems deliver this once - stock, trading, campaigns and the
    certificate register - and it does not move afterwards. Unlike the tape it
    is released at load rather than on the clock: there is no sense in which a
    reader is being shown the future by being told what a depot holds, and a
    graph that only fills in once somebody presses play would be a graph nobody
    sees.

    That is also why the arrivals are written here rather than by ``advance``.
    The estate's MCP servers read the ``arrivals`` table, so a reference event
    with no arrival row is an event no back-office console and no graph loader
    can reach - it would exist and be unreadable.

    A missing file is not an error. The pack is generated by
    ``scripts/generate_backoffice.py`` and a checkout that has not run it yet
    should still boot, with a graph that says it holds nothing rather than a
    front door that will not open.
    """
    source = path or (baseline_mod.data_dir() / "backoffice.jsonl")
    if not source.exists():
        return {"loaded": 0, "total": 0, "skipped": True, "missing": True}

    conn = db.connect()
    if reset:
        # The arrivals first, while the events they point at still exist.
        conn.execute(
            "DELETE FROM arrivals WHERE event_id IN"
            " (SELECT id FROM events WHERE lane = ?)", (LANE_REF,))
        conn.execute("DELETE FROM events WHERE lane = ?", (LANE_REF,))
        conn.commit()
    ...
    with source.open(encoding="utf-8") as fh:
        for line in fh:
            ...
            e = json.loads(line)
            rows.append((e["id"], e["seq"], e["ts"], e["type"], e["source"],
                         db.dumps(e["payload"]), e.get("body"), now))
            events.append(Event(id=e["id"], seq=e["seq"],
                                ts=datetime.fromisoformat(e["ts"]),
                                type=e["type"], source=e["source"],
                                payload=e["payload"], body=e.get("body"),
                                lane=LANE_REF))

    with db.transaction() as c:
        c.executemany(
            "INSERT OR REPLACE INTO events (id, seq, ts, type, source, payload,"
            " body, released_at, lane) VALUES (?,?,?,?,?,?,?,?,'REF')", rows)

    from sc.estate import delivery

    delivery.deliver_reference(events)
    return {"loaded": len(rows), "total": len(rows), "skipped": False}
